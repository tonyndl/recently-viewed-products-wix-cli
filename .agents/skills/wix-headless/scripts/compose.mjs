...
if (process.stdin.isTTY)
  die("NO_INPUT", "expected the compose input JSON on stdin");
let input;
try {
  const raw = readFileSync(0, "utf8").trim();
  if (!raw)
    die("EMPTY_INPUT", "stdin was empty — pass the compose input JSON object");
...
function designMdToTokens(fm) {
  ...
  for (const [k, v] of Object.entries(fm.colors ?? {}))
    colors[COLOR_ROLE_TABLE[k] ?? k] = v;
  const typ = fm.typography ?? {};
  const famOf = (...keys) => {
    for (const key of keys)
      if (typ[key] && typ[key].fontFamily) return typ[key].fontFamily;
    return undefined;
  };
  const fonts = {};
  ...
  return {
    ...
    ...(typeof fm.googleFontsHref === "string"
      ? { googleFontsHref: fm.googleFontsHref }
      : {}),
  };
}
...
const designMdPath = isAbsolute(designMdRel)
  ? designMdRel
  : join(projectDir, designMdRel);
if (!existsSync(designMdPath))
  die(
    "DESIGN_MD_MISSING",
    `no DESIGN.md at ${designMdPath} — the Designer must author it first`,
  );
const fm = parseFrontmatter(readFileSync(designMdPath, "utf8"));
if (!fm)
  die("DESIGN_MD_BAD", `could not parse YAML frontmatter from ${designMdPath}`);
...
const packsWithComponents = Array.isArray(input.packsWithComponents)
  ? input.packsWithComponents
  : [];
const disabledPacks = Array.isArray(input.disabledPacks)
  ? input.disabledPacks
  : [];
...
// core role), then warm/soft derivations described in COMPOSE § Token contract.
  ...
  colors,
  "paper-warm",
  mix(colors.paper, colors.accent, 0.06),
  "--color-",
); // paper warmed
...
for (const [k, v] of Object.entries(SPACING_DEFAULTS))
  ensure(spacing, k, v, "--spacing-");
...
for (const [k, v] of Object.entries(CONTAINER_DEFAULTS))
  ensure(containers, k, v, "--container-");

// Assemble the @theme block, grouped + ordered for readability.
const themeLines = [];
const emitGroup = (prefix, obj) => {
  for (const [k, v] of Object.entries(obj))
    themeLines.push(`  ${prefix}${k}: ${v};`);
};
...
const REQUIRED = [
  ...["paper", "paper-warm", "ink", "mute", "rule", "accent"].map(
    (k) => `--color-${k}`,
  ),
  "--font-display",
  "--font-body",
  ...["2xs", "xs", "sm", "md", "lg", "xl", "2xl", "3xl", "4xl"].map(
    (k) => `--spacing-${k}`,
  ),
  "--radius-sm",
  "--radius-md",
...
for (const tok of REQUIRED) {
  if (!present.has(tok))
    errors.push({ code: "MISSING_REQUIRED_TOKEN", token: tok });
}
...
for (const [k, v] of Object.entries(containers)) {
  if (spacingValues.has(v))
    errors.push({
      code: "CONTAINER_EQUALS_SPACING",
      token: `--container-${k}`,
      value: v,
    });
}
...
function buildGoogleHref() {
  if (
    typeof designTokens.googleFontsHref === "string" &&
    designTokens.googleFontsHref.trim()
  ) {
...
// ── 1. global.css ──────────────────────────────────────────────────────────────
{
  ...
  if (out === skeleton)
    die(
      "ANCHOR_THEME",
      "could not locate the @theme block in global.css skeleton",
    );
...
// ── 2. astro.config.mjs (anchored MERGE, fail loud on missing anchor) ─────────
{
  ...
  if (src === null)
    die(
      "SCAFFOLD_NOT_COMPLETE",
      `astro.config.mjs not found at ${dest} after retries`,
    );

  const hasImport =
    /import\s+tailwindcss\s+from\s+["']@tailwindcss\/vite["']/.test(src);
  ...
  if (!hasImport) {
    ...
    if (!last)
      die(
        "ANCHOR_IMPORT",
        "no import statement found in astro.config.mjs to anchor the @tailwindcss/vite import",
      );
  ...
  if (!hasCall) {
    if (/plugins\s*:\s*\[/.test(src)) {
      ...
      die(
        "ANCHOR_VITE",
        "could not find vite.plugins, a vite block, or defineConfig({ to register @tailwindcss/vite",
      );
    }
  }
  ...
  const alreadyGuarded = /\(globalThis\)\)\.process\?\.env\?\.NODE_ENV/.test(
    src,
  );
  const bareRe =
    /const\s+isBuild\s*=\s*process\.env\.NODE_ENV\s*===?\s*["']production["']\s*;/;
  if (!alreadyGuarded) {
    if (bareRe.test(src)) {
      ...
      die(
        "ANCHOR_PROCESS_ENV",
        "found a bare process.env reference but not the expected `const isBuild = process.env.NODE_ENV …` line to guard",
      );
    }
...
// ── 4. Navigation.astro ──────────────────────────────────────────────────────────
{
  ...
  out = out.replaceAll(
    "{{shell.navBrandMark}}",
    shell.navBrandMark ?? brandName,
  );
  const links = navItems
    .map(
      (l) =>
        `        <li class="site-nav-item"><a href="${escAttr(l.href)}">${l.label}</a></li>`,
    )
    .join("\n");
