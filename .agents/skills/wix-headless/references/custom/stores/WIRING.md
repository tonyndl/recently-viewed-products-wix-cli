    ...
    try {
      ...
      grid.replaceChildren(
        ...items.map((p) => {
          ...
          if (img) img.src = p.media?.mainMedia?.image?.url ?? "";
          const name = card.querySelector("h3.name");
          if (name) name.textContent = p.name ?? "";
          const price = card.querySelector("span.price");
          if (price) price.textContent = p.priceData?.formatted?.price ?? "";
          ...
          if (link && p.slug) link.href = `/product/${p.slug}`;
          return card;
        }),
      );
    } catch (err) {
