...
function applies(scopes: any[] | undefined, productId: string): boolean {
  ...
  for (const s of scopes) {
    ...
    if (
      Array.isArray(ids) &&
      ids.length === 0 &&
      s?.catalogItemFilter?.catalogAppId
    )
      return true;
...
function ruleApplies(rule: any, productId: string): boolean {
  ...
  for (const v of values) {
    if (v?.targetType === "SPECIFIC_ITEMS") {
      ...
      if (applies(v?.buyXGetYInfo?.customerBuys?.scopes, productId))
        return true;
      if (applies(v?.buyXGetYInfo?.customerGets?.scopes, productId))
        return true;
    }
  }
  return false;
...
export async function fetchLiveOffers(): Promise<any[]> {
  try {
    ...
    return list.filter(
      (r: any) => r?.active !== false && r?.status !== "EXPIRED",
    );
  } catch (err) {
