...
export default function BookingForm({
  ...
  const maxParty =
    serviceType === "CLASS"
      ? Math.max(1, slot.bookableCapacity ?? slot.remainingCapacity ?? 1)
      : 1;
  const partyOptions = Array.from(
    { length: Math.min(maxParty, 8) },
    (_, i) => i + 1,
  );
      ...
      : // APPOINTMENT: scheduleId + explicit start/end/timezone/location.
        {
          ...
          ...(slot.locationId
            ? {
                location: {
                  _id: slot.locationId,
                  locationType: slot.locationType as any,
                },
              }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        /SESSION_CAPACITY_EXCEEDED/i.test(full) ||
        /no available spots/i.test(full)
      ) {
        ...
        setError(
          "This class is full and we couldn't add you to the waitlist — please try again later.",
        );
        setSubmitting(false);
        return;
      }
      ...
      <div className="booking-form">
        <div className="booking-form-header">
          ...
          <p className="booking-form-subtitle">
            {serviceName} · {slotDisplay(slot)}
          </p>
        </div>
        <p>
          We'll email <strong>{email.trim()}</strong> if a spot opens up.
        </p>
    ...
    <form className="booking-form" onSubmit={handleSubmit}>
      <div className="booking-form-header">
        ...
        {isWaitlist && (
          <p className="booking-form-note">
            This session is full — join the waitlist and we'll notify you if a
            spot opens.
          </p>
        )}
      </div>
      ...
      <div className="booking-field-row">
        <div className="booking-field">
          <label
            className="booking-label booking-label--required"
            htmlFor="bf-first"
          >
            First name
          </label>
        ...
        <div className="booking-field">
          <label className="booking-label" htmlFor="bf-last">
            Last name
          </label>
          <input
            id="bf-last"
      ...
      <div className="booking-field">
        <label className="booking-label" htmlFor="bf-phone">
          Phone
        </label>
        <input
          id="bf-phone"
      ...
      {!isWaitlist && maxParty > 1 && (
        <div className="booking-field">
          <label className="booking-label" htmlFor="bf-party">
            Number of spots
          </label>
          <select
            id="bf-party"
