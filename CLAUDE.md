...
export default app().use(countdownTimer).use(dataExtension);
```
...
export const fetchItems = () =>
  httpGet(`${baseApiUrl}/api/items`).then(
    (res) => res.json() as Promise<ItemRecord[]>,
  );

export const postItem = (name: string, order: number) =>
  httpPost(`${baseApiUrl}/api/items`, { name, order }).then(
    (res) => res.json() as Promise<ItemRecord>,
  );

export const patchItem = (id: string, name: string, order?: number) =>
  httpPatch(`${baseApiUrl}/api/items`, { _id: id, name, order }).then(
    (res) => res.json() as Promise<ItemRecord>,
  );
...
export const getAPI = (
  ...
  items
    ...
    .catch((error: unknown) =>
      customJson({ error: String(error) }, { status: 500 }),
    );
