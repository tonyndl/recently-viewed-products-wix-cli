...
export const openRatePopup = (reviewUrl: string = REVIEW_URL) => {
  if (typeof window === 'undefined') return;
  ensureDefined();
  let el = document.querySelector(TAG) as (HTMLElement & { open: (src: string) => void }) | null;
  if (!el) {
    el = document.createElement(TAG) as HTMLElement & { open: (src: string) => void };
