...
export const fetchPlan = (): Promise<PlanStatus> =>
  httpGet(`${baseApiUrl}/api/check-plan`).then((r) => r.json() as Promise<PlanStatus>);
  ...
  appPlans
    ...
    .then((result) => {
      ...
      return {
        ...
        showPriceWithTax: (result as any).taxSettings?.showPriceWithTax ?? false,
      };
    })
...
export const fetchStoreProductCount = async (): Promise<number> => {
  try {
    const res = await productsV3.queryProducts().limit(1).find();
    const count = (res as { totalCount?: number }).totalCount ?? res.items.length ?? 0;
