...
export const ImageRatioPicker: FC<Props> = ({ value, isPremium, onChange, onUpgrade }) => {
  // Free users effectively use a free ratio regardless of any premium ratio
  // saved during a previous premium session.
  const effectiveValue = isPremium || FREE_RATIOS.includes(value) ? value : FREE_RATIOS[0];
            ...
            <div style={selected ? styles.thumbSelected : locked ? styles.thumbLocked : styles.thumb}>
              {locked ? (
                <div style={styles.thumbDim}>
                  <ImageRatioThumb kind={o.id} color={SHAPE} contrast={CARD_BG} />
                </div>
              ) : (
                <ImageRatioThumb
                  kind={o.id}
