...
export const TextPositionPicker: FC<Props> = ({ value, isPremium, onChange, onUpgrade }) => {
  // Free users effectively use a free position regardless of any premium
  // position saved during a previous premium session.
  const effectiveValue = isPremium || FREE_TEXT_POSITIONS.includes(value) ? value : FREE_TEXT_POSITIONS[0];
            ...
            <div style={selected ? styles.thumbSelected : locked ? styles.thumbLocked : styles.thumb}>
              {locked ? (
                <div style={styles.thumbDim}>
                  <TextPositionThumb kind={o.id} color={SHAPE} contrast={CARD_BG} />
                </div>
              ) : (
                <TextPositionThumb
                  kind={o.id}
