...
async function init() {
    ...
    setTimeout(async() => {
        ...
        $w('#noProductsText').delete();
        let itemsToDisplay = await wixData.query('Stores/Products').hasSome('slug', JSON.parse(localItem)).fields('mainMedia', 'name', 'id', 'formattedDiscountedPrice', 'productPageUrl').find().then(res => res.items);
...
async function showDefault() {
    ...
    let itemsToDisplay = await wixData.query('Stores/Products').fields('mainMedia', 'name', 'id', 'formattedDiscountedPrice', 'productPageUrl').find().then(res => res.items);
